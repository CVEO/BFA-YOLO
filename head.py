...
class TDATH(nn.Module):
    ...
    def __init__(self, nc=7, hidc=256, ch=()):
        # 7 categories
        ...
        self.cls_decomp = TaskSpliter(hidc // 2, 2, 16)
        self.reg_decomp = TaskSpliter(hidc // 2, 2, 16)
        self.Conv_D = Conv_D(hidc // 2, hidc // 2)
        self.Generator_conv_mask = nn.Conv2d(hidc, 3 * 3 * 3, 3, padding=1)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            # reg alignment
            offset_and_mask = self.Generator_conv_mask(feat)
            offset = offset_and_mask[:, :self.offset_dim, :, :]
            mask = offset_and_mask[:, self.offset_dim:, :, :].sigmoid()
            reg_feat = self.Conv_D(reg_feat, offset, mask)
